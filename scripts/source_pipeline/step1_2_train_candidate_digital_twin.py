...
MODEL_DIR = PROJECT_ROOT / "models" / "digital_twin_candidate_baseline"
RESULT_DIR = PROJECT_ROOT / "results" / "step1_2"
...
def main():
    ...
    print(f"Train shape: {X_train_s.shape}, {Y_train_s.shape}")
    print(f"Validation shape: {X_val_s.shape}, {Y_val_s.shape}")
    print(f"Test shape: {X_test_s.shape}, {Y_test_s.shape}")

    model = CandidateDigitalTwinMLP(input_dim=input_dim, output_dim=output_dim).to(device)
    ...
    best_path = MODEL_DIR / "candidate_dt_mlp.pt"
    ...
    config = {
        ...
        "device": device,
        "note": (
            "Candidate-aware baseline digital twin trained on previous-state "
            "features plus candidate observed slicing actions."
        ),
    }
    ...
    with open(MODEL_DIR / "step1_2_metrics.json", "w", encoding="utf-8") as f:
        json.dump(metrics, f, indent=2)

    with open(RESULT_DIR / "step1_2_report.md", "w", encoding="utf-8") as f:
        f.write("# Step 1.2 Candidate-Aware Digital Twin Baseline Report\n\n")
        ...
        f.write("- This is the first formal candidate-aware digital twin baseline.\n")
        f.write("- Inputs include previous-step state features and candidate slicing actions.\n")
        f.write("- Its performance is expected to be lower than Step 0.6 sanity model.\n")
        f.write("- The next step will train an ensemble/dropout version for uncertainty estimation.\n")
