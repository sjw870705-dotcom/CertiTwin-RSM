...
MODEL_DIR = PROJECT_ROOT / "models" / "ppo_raw_controller"
RESULT_DIR = PROJECT_ROOT / "results" / "step2_5"
...
MODEL_PATH = MODEL_DIR / "ppo_policy.pt"
TRAIN_LOG = MODEL_DIR / "step2_5_ppo_training_log.csv"
METRICS_JSON = MODEL_DIR / "step2_5_ppo_metrics.json"

RAW_ACTIONS_CSV = EVAL_DIR / "step2_5_ppo_raw_actions.csv"
PAIRWISE_CSV = EVAL_DIR / "step2_5_ppo_raw_vs_shielded_pairwise.csv"
SUMMARY_CSV = EVAL_DIR / "step2_5_ppo_shielding_summary.csv"
REPORT_MD = RESULT_DIR / "step2_5_report.md"
...
def gather_action_values(matrix, group_indices, actions):
    return matrix[group_indices, actions]


def train_ppo(env, device):
    state = torch.from_numpy(env["state"]).to(device)
    reward = torch.from_numpy(env["reward"]).to(device)

    train_idx = env["train_idx"]
    state_dim = env["state"].shape[1]
    action_dim = env["reward"].shape[1]
    ...
    for update in range(1, NUM_UPDATES + 1):
        ...
        if update % 10 == 0 or update == 1:
            eval_train = evaluate_policy(model, env, env["train_idx"][:min(5000, len(env["train_idx"]))], device)
            eval_val = evaluate_policy(model, env, env["validation_idx"], device)
...
@torch.no_grad()
def evaluate_policy(model, env, indices, device):
    actions = select_actions(model, env["state"], indices, device, deterministic=True)

    reward = env["reward"][indices, actions]
...
def load_trained_model(env, device):
    state_dim = env["state"].shape[1]
    action_dim = env["reward"].shape[1]
...
def build_raw_actions_csv(model, env, device):
    ...
    for i, gid_index in enumerate(test_idx):
        ...
        rows.append({
            ...
            "raw_reward": float(env["reward"][gid_index, action]),
        })
...
def shield_ppo_actions(raw_df, candidates):
    ...
    for _, raw in raw_df.iterrows():
        ...
        if int(raw_cand["cert_joint_safe"]) == 1:
            ...
            if len(cert_safe) > 0:
                # Use same projection score as previous shield.
        ...
        pair_rows.append({
            "controller": "PPO",
            "shielded_controller": "CertiTwin-PPO",
...
def main():
    ...
    model, logs = train_ppo(env, device)

    train_metrics = evaluate_policy(model, env, env["train_idx"], device)
    val_metrics = evaluate_policy(model, env, env["validation_idx"], device)
    test_metrics = evaluate_policy(model, env, env["test_idx"], device)

    raw_df = build_raw_actions_csv(model, env, device)

    print(f"Reading candidates for shielding: {CANDIDATES_FILE}")
    candidates = pd.read_csv(CANDIDATES_FILE)

    pair_df = shield_ppo_actions(raw_df, candidates)
    ...
    with open(REPORT_MD, "w", encoding="utf-8") as f:
        f.write("# Step 2.5 PPO Raw Controller and CertiTwin-PPO Report\n\n")
        ...
        f.write("- Raw PPO and CertiTwin-PPO use the same trained PPO checkpoint.\n")
        f.write("- CertiTwin only modifies PPO-selected actions at execution time.\n")
        f.write("- This is the first DRL paired-controller comparison.\n")
